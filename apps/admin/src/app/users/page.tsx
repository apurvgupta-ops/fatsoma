  ...
  return (
    <AuthenticatedLayout>
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-4 pb-16 pt-12 sm:px-6 lg:px-8">
        ...
        {message && (
          <div className={`rounded-xl border p-4 ${message.type === "success" ? "border-gold/40 bg-gold/10 text-gold" : "border-rose-500/40 bg-rose-500/10 text-rose-300"}`}>
            {message.text}
          </div>
        )}
                ...
                <tbody className="divide-y divide-border">
                  {users.map((u) => (
                    <tr key={u.id} className="transition hover:bg-surface/40">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${u.isActive ? "bg-gold/10 text-gold border border-gold/40" : "bg-border/40 text-cream/60 border border-border"}`}>
