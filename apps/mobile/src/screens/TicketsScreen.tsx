...
const STATUS_STYLES: Record<string, { bg: string; text: string }> = {
  active: { bg: "rgba(201,169,110,0.12)", text: colors.gold.DEFAULT },
  listed: { bg: "rgba(232,213,163,0.12)", text: colors.gold.light },
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardMeta}>
          <View style={styles.metaRow}>
            <Ionicons name="pricetag-outline" size={14} color={colors.gold.light} />
            <Text style={styles.metaText}>{item.ticketBatchName}</Text>
          </View>
          {item.eventDate && (
            <View style={styles.metaRow}>
              <Ionicons name="calendar-outline" size={14} color={colors.gold.light} />
          ...
          {item.venueName && (
            <View style={styles.metaRow}>
              <Ionicons name="location-outline" size={14} color={colors.gold.light} />
              <Text style={styles.metaText}>
                {item.venueName}
                {item.city ? `, ${item.city}` : ""}
              </Text>
        ...
        <View style={styles.cardActions}>
          ...
          {item.status === "active" && item.allowResale && (
            ...
            >
              <Ionicons name="swap-horizontal" size={16} color={colors.gold.DEFAULT} />
              <Text style={styles.resaleBtnText}>List for Resale</Text>
            </Pressable>
          )}

          {item.status === "listed" && (
            <Pressable style={styles.cancelBtn} onPress={() => handleCancelListing(item)}>
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      {tickets.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>Your purchased tickets will appear here</Text>
        </View>
      ) : (
        <FlatList
          data={tickets}
            ...
            {resaleTicket && (
              <>
                ...
                <View style={styles.modalInfoRow}>
                  ...
                  <Text style={[styles.modalInfoValue, { color: colors.gold.DEFAULT }]}>
...
const styles = StyleSheet.create({
  ...
  resaleBtnText: { fontSize: 13, fontWeight: "600", color: colors.gold.DEFAULT },
  cancelBtn: {
    flex: 1,
    flexDirection: "row",
    alignItems: "center",
