...
import WhatHappensSection from "@/components/unbuy/WhatHappensSection";
import AtomicTransferSection from "@/components/unbuy/AtomicTransferSection";
import EmptyTicketsAnimation from "@/components/unbuy/EmptyTicketsAnimation";
...
export default function UnbuyPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push("/login?redirect=/unbuy");
      return;
    }
    fetchTickets();
  ...
  const examplePrice =
    activeTickets[0]?.purchasePrice ??
    resaleTickets[0]?.purchasePrice ??
    20;

  if (authLoading || loading) {
    return (
      <div className={`relative min-h-screen bg-void ${SITE_HEADER_OFFSET}`}>
        <Header />
  ...
  return (
    ...
    >
      ...
      <div className="relative z-[2]">
        <WhatHappensSection price={Math.round(examplePrice)} />

        <div className="mx-auto max-w-[900px] px-6 pb-20 sm:px-12">
          {hasAnyTickets ? (
            <>
              <p className="font-jost mb-6 text-[11px] tracking-[0.2em] text-gold uppercase">
                YOUR TICKETS
              </p>

              <div className="mb-6 flex gap-1 border border-border bg-[#141414] p-1 w-fit rounded-lg">
                <button
                  onClick={() => setActiveTab("active")}
                  className={`cursor-pointer rounded-md px-4 py-2 text-xs font-jost tracking-[0.08em] uppercase transition-all ${
                    activeTab === "active"
                      ? "bg-gold text-void"
                      : "text-[#888888] hover:text-cream"
                  }`}
                >
              ...
              <div className="relative mb-5 max-w-sm">
                ...
                <input
                  ...
                  className="otl-search-input w-full border-b border-border bg-transparent py-2.5 pl-9 pr-4 text-sm outline-none"
                />
              </div>
            ...
            <EmptyTicketsAnimation />
          )}
        </div>

        <AtomicTransferSection />
      </div>
